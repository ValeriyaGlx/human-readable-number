module.exports = function toReadable (number) {
   ...
   if(n<=99){ 
     n = n.toString().split('');
     return (tenner[n[0]] + ' ' + ones[n[1]]).trim()
   }

   if(n<=999){
       n = n.toString().split('');
   if(n[1]==='0') return (ones[n[0]] + ' hundred ' + ones[n[2]]).trim();

   if(n[1]==='1') return (ones[n[0]] + ' hundred ' + tens[n[2]]).trim();

   return (ones[n[0]] + ' hundred ' + tenner[n[1]] + ' ' + ones[n[2]]).trim();
   } 
